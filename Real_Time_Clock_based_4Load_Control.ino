...
void setup() {
  ...
  lcd.clear();
  lcd.setCursor (0, 0);
  lcd.print(" NTS Electronics ");
  lcd.setCursor (0, 1);
  lcd.print("  03000-695646 ");
  delay (3000);
  lcd.clear();
...
void ReadEeprom() {
  ...
  if (mode == 1) {
    StartHH = Start1HH, StartMM = Start1MM, FinishHH = Finish1HH, FinishMM = Finish1MM;
  }
  if (mode == 2) {
    StartHH = Start2HH, StartMM = Start2MM, FinishHH = Finish2HH, FinishMM = Finish2MM;
  }
  if (mode == 3) {
    StartHH = Start3HH, StartMM = Start3MM, FinishHH = Finish3HH, FinishMM = Finish3MM;
  }
  if (mode == 4) {
    StartHH = Start4HH, StartMM = Start4MM, FinishHH = Finish4HH, FinishMM = Finish4MM;
  }
